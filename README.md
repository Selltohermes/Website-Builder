...
cash home buyer. Static HTML/CSS/JS — no build step, no dependencies, no framework.
...
| **Netlify Forms** | `/` — also add `netlify` and a hidden `form-name` input to the `<form>` | Free if you host on Netlify |
...
- **`privacy.html` / `terms.html`** — set the "Last updated" date, and disclose any
...
for speed. Keep that — it's what keeps aggressive marketing claims defensible.
